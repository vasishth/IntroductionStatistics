...
\title[Lecture 5]{Introduction to statistics: Matrix formulatuon of the linear model}
...
$\ell(\theta)$ or $\log \Lambda$ refers to the log likelihood. $\Delta$ refers to the ratio (or difference, in log likelihood) of likelihood.
