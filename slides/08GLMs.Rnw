...
\title[Lecture 6]{Introduction to statistics: Generalized linear models (logistic regression)}
...
The interpretation of the coefficients is making little sense here.
...
\section{Residuals in GLMs}

\begin{frame}[fragile]\frametitle{Residuals in GLMs}
...
<<>>=
fm_skip_lmer2<-glmer(skip ~ scale(word_complex,scale=FALSE)+
                      scale(SC,scale=FALSE) +(1|subj)+(1|item),
               family=binomial(),hindi)
