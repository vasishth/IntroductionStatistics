...
The generalized inverse (details require matrix algebra, which we will just turn to):
...
You can see that the model or design matrix X is based on the generalized inverse of the hypothesis matrix.
